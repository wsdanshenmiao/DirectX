...
namespace DSM {
	...
	void DrawBox::OnUpdate(const CpuTimer& timer)
	{
		...
		static float theta = 0.f, phi = 0.f, radius = 6.f, scale = 1.f, fov = XM_PIDIV2, dx = 0, dy = 0;
		...
		if (ImGui::Begin("Use ImGui"))
		{
			...
			ImGui::Text("Position: (%.1f, %.1f, 0.0)", dx, dy);
		...
		if (!ImGui::IsAnyItemActive())
		{
			...
			{
				dx += io.MouseDelta.x * 0.01f;
				dy -= io.MouseDelta.y * 0.01f;
			}
			// 鼠标右键拖动旋转
	...
	bool DrawBox::InitResource()
	{
		...
		// 创建常量缓冲区及其视图
		m_ObjCB = std::make_unique<UploadBuffer<ObjConstants>>(m_D3D12Device.Get(), 1, true);
